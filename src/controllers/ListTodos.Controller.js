...
const TodosList= async(req,res)=>{
    ...
    try {
        const data= req.body
        console.log(data);
          if(!data.id) res.status(400).send({status:false,msg:"Please provide todos id"})
          if(!data.title)  res.status(400).send({status:false,msg:"Please provide todos title"})
          
          const CreateTodo= await TodoListSchema.create(data)
          if(!CreateTodo) res.status(400).send({status:false,msg:"dotu is not created" })
 ...
 export const GetTodoList=async(req,res)=>{
    ...
    try {
        const getData=await TodoListSchema.find().select({id:1,title:1,_id:0})
     
    if(!getData) res.status(400).send({status:false,msg:"todo List is Empty"})();
